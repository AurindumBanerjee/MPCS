...
# Bloom filter tuning for fast seen-before checks on summaries.
# max_elements is an expected upper bound of stored experiences.
# error_rate controls false-positive probability.
BLOOM_MAX_ELEMENTS = 10000
...
class MemorySystem:
    ...
    def __init__(self):
        ...
        self._bloom = BloomFilter(
            max_elements=BLOOM_MAX_ELEMENTS,
            error_rate=BLOOM_ERROR_RATE,
        )

    @staticmethod
    def _summary_key(summary: tuple) -> str:
        """Deterministic summary serialization for Bloom membership tests."""
        return repr(summary)

    def store(self, summary: tuple, action: str, reward: float, step: int) -> None:
        self._bloom.add(self._summary_key(summary))
        self._store.append({
            "summary": summary,
            "action":  action,
            "reward":  reward,
    ...
    def maybe_seen(self, summary: tuple) -> bool:
        """Fast probabilistic membership check for summary existence."""
        return self._summary_key(summary) in self._bloom
    ...
    def retrieve(self, summary: tuple, k: int = TOP_K_MEMORIES) -> list[dict]:
        ...
        if not self.maybe_seen(summary):
            return []
...
def compute_novelty(summary: tuple, memory: MemorySystem) -> float:
    ...
    if len(memory) == 0:
        return 1.0
    if not memory.maybe_seen(summary):
        return 1.0
